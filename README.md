...
Run migrations `022_engineering_prompt_workflow.sql`, `023_workflow_automation.sql`, and `024_github_app_installations.sql`. Configure the GitHub App Setup URL as `https://www.closespan.com/api/integrations/github/callback` and set `GITHUB_APP_INSTALL_URL=https://github.com/apps/closespan/installations/new`. The authenticated callback verifies the installation with GitHub, binds it to the initiating workspace administrator through a signed one-time attempt, and synchronizes only repositories selected in GitHub. Configure `GITHUB_APP_ID`, `GITHUB_APP_PRIVATE_KEY`, `AGENT_EXECUTOR_URL`, `AGENT_EXECUTOR_SHARED_SECRET`, and `CLOSESPAN_INTERNAL_BASE_URL` in the Vercel application.
