...
Run migrations `022_engineering_prompt_workflow.sql` through `025_github_app_webhooks.sql`. Configure the GitHub App Setup URL as `https://www.closespan.com/api/integrations/github/callback`, its active Webhook URL as `https://www.closespan.com/api/integrations/github/webhook`, and set `GITHUB_APP_INSTALL_URL=https://github.com/apps/closespan/installations/new`. Subscribe only to installation/repository lifecycle events and pull-request events. The authenticated callback verifies the installation, binds it to the initiating workspace administrator through a signed one-time attempt, and synchronizes only repositories selected in GitHub. The webhook independently verifies GitHub's SHA-256 signature over the raw payload, deduplicates the delivery ID, resynchronizes previously bound installations, and audits tracked draft-PR changes without merging, deploying, or changing release verification. Configure `GITHUB_APP_ID`, `GITHUB_APP_PRIVATE_KEY`, `GITHUB_WEBHOOK_SECRET`, `AGENT_EXECUTOR_URL`, `AGENT_EXECUTOR_SHARED_SECRET`, and `CLOSESPAN_INTERNAL_BASE_URL` in the Vercel application.
...
UI changes receive an additional immutable verification bundle during the browser-enabled Tenki implementation run. A fenced `closespan-ui-verification` JSON block in the PDD release instructions may define up to eight declarative journey/viewport captures; prose-only instructions use the safe desktop/mobile root-page contract. The production release verifier repeats those journeys in a fresh outbound-only Tenki VM, blocks browser requests outside the configured application origin, checks selectors/text/navigation, browser errors and basic accessibility, captures screenshots, and compares bounded DOM layout geometry with the PR-head baseline. It cannot run generated shell commands, merge, deploy, or write repository data. Configure `RELEASE_ENVIRONMENT_URLS`, `RELEASE_VERIFIER_ALLOWED_HOSTS`, one of `TENKI_RELEASE_VERIFIER_IMAGE` or `TENKI_RELEASE_VERIFIER_SNAPSHOT_ID`, and optionally `RELEASE_VERIFIER_STORAGE_STATE_JSON` for a dedicated synthetic account. The durable Cloudflare queue boundary lives in `workers/release-verifier` and carries only tenant-scoped job references; immutable evidence and credentials never enter the queue payload.
