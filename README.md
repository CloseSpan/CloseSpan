...
Immediately after creating or recovering a CloseSpan draft PR, the publisher requests one Tenki Code Reviewer pass by posting an idempotency-marked `@tenki-reviewer` comment. Retries reuse the marker and do not intentionally request another paid review. The repository must be connected to the separately installed Tenki Code Reviewer GitHub App; Tenki bills Code Reviewer usage independently from CloseSpan's Tenki runner and sandbox usage. A temporary review-request failure is logged but does not invalidate an implementation that already passed independent verification or remove its draft PR.
