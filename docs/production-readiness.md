...
- Set the GitHub App Setup URL to `https://closespan.com/api/integrations/github/callback`. Exercise the complete signed, one-time handoff from an authenticated workspace admin, validate the returned installation through the GitHub App API, and confirm repository updates cannot cross tenants or re-use an expired attempt.
