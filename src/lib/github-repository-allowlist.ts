...
export async function listGithubRepositoryAuthorizations(orgId: string): Promise<GithubRepositoryAuthorization[]> {
  ...
  }>(`SELECT id,installation_id::text,repository,default_branch,workspace_selected,active
        FROM github_repository_allowlists WHERE org_id=$1 ORDER BY repository`, [orgId]);
  return result.rows.map((row) => ({ id: row.id, installationId: row.installation_id, repository: row.repository, defaultBranch: row.default_branch, workspaceSelected: row.workspace_selected, active: row.active }));
}
