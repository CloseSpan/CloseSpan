...
export function assertGithubActionsRunIdentity(input: {
  ...
  const expectedWorkflowIdentity = `${input.repository}/${input.workflowPath}`;
  const claimedWorkflowIdentity = input.claims.workflow_ref.split("@")[0];
  const exactCommitFallback = Boolean(
    input.expectedSha
    && input.claims.sha.toLowerCase() === input.expectedSha.toLowerCase()
    && claimedWorkflowIdentity === expectedWorkflowIdentity,
  );
