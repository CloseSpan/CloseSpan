...
export async function captureTenkiUiBaseline(
  ...
  for (const journey of frontend.journeys) {
    for (const viewport of frontend.viewports) {
...
export function uiBaselinePassed(
  ...
  return baseline.captures.every((capture) =>
    ...
    && (!frontend.failOnConsoleError || capture.consoleErrors.length === 0)
    && (!frontend.failOnPageError || capture.pageErrors.length === 0)
