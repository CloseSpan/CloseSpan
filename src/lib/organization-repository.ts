...
async function queryOrganizationMemberships(
  queryable: Pick<PoolClient, "query">,
  normalizedEmail: string,
): Promise<OrganizationMembership[]> {
  const result = await queryable.query<MembershipRow>(membershipLookupSql, [
    normalizedEmail,
  ]);
...
export async function listOrganizationMemberships(
  ...
  return queryOrganizationMemberships(databasePool(), normalizedEmail);
}
...
export async function createOrganization(
  ...
  // Validate before opening the transaction so invalid requests never touch the DB.
  requiredTrimmed(input.name, "Organization name", 120);
...
/**
 * Returns the user's organizations, creating a private starter workspace when
 * their verified identity has no membership yet. The transaction-scoped lock
 * prevents concurrent first requests from provisioning duplicate tenants.
 */
export async function ensureOrganizationMemberships(
  email: string,
  ...
  if (!normalizedEmail || !normalizedEmail.includes("@"))
    throw new Error("A valid verified email is required to create a workspace");
  const creatorName = (
    displayName?.trim() ||
  ...
  return transaction(async (client) => {
    ...
    const created = await createOrganizationWithClient(client, {
      name: workspaceName,
      productName: null,
      productUrl: null,
      productDescription: null,
      creator: { name: creatorName, email: normalizedEmail },
    });
    return [
      {
