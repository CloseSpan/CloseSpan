...
describe("GitHub installation persistence", () => {
  ...
  it("discovers authorized repositories without selecting them for the workspace", async () => {
    await expect(
      connectGithubInstallation("attempt-1", "org-1", actor, installation),
    ).resolves.toEqual({
      repositoryCount: 0,
      availableRepositoryCount: 2,
      repositories: [],
    });
    ...
    const repositoryInserts = database.client.query.mock.calls.filter(([query]) =>
      sql(query).includes("INSERT INTO github_repository_allowlists"),
    );
    expect(repositoryInserts.every(([, parameters]) => parameters?.[5] === false)).toBe(true);
  });
  ...
  it("preserves only this workspace's prior repository selection when reconnecting", async () => {
    database.client.query.mockImplementation(async (query: unknown) => {
      ...
      if (sql(query).includes("SELECT repository FROM github_repository_allowlists")) {
        return { rows: [{ repository: "acme/web" }], rowCount: 1 };
      }
      return { rows: [], rowCount: 1 };
    });
  ...
  it("preserves workspace repository selection during installation webhooks", async () => {
    ...
    expect(inserts.find(([, parameters]) => parameters?.[3] === "acme/api")?.[1]?.[5]).toBe(true);
    expect(inserts.find(([, parameters]) => parameters?.[3] === "acme/web")?.[1]?.[5]).toBe(false);
  });
  ...
  it("sets an explicit workspace repository subset after verifying GitHub access", async () => {
    ...
    expect(inserts).toHaveLength(2);
    expect(inserts.find(([, parameters]) => parameters?.[3] === "acme/web")?.[1]?.[5]).toBe(true);
    expect(inserts.find(([, parameters]) => parameters?.[3] === "acme/api")?.[1]?.[5]).toBe(false);
  });
