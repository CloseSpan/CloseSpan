...
export interface CustomerView {
  ...
  customerSinceKnown: boolean;
  signals: number;
  openProblems: number;
  churnRisk: string;
  origin: "demo" | "manual" | "integration";
  sourceNames: string[];
}
...
async function memoryData(orgId: string): Promise<WorkspaceData> {
  ...
  const savedPolicy = getMemoryWorkspacePolicy(orgId);
...
export async function getWorkspaceData(orgId: string): Promise<WorkspaceData> {
  ...
  const [analytics, feedbackResult, problemResult, investigationResult, integrationResult, customerResult, settingsResult, membersResult, promptResult, modelRunResult, ai] = await Promise.all([
    ...
    pool.query<{ id:string; name:string; tier:string; arr:number; arr_source:string; customer_since:number; customer_since_known:boolean; churn_risk:string; origin:CustomerView["origin"]; source_names:string[]; signals:number; open_problems:number }>(`SELECT a.id,a.name,a.tier,a.arr,a.arr_source,a.customer_since,
        a.customer_since_known,a.churn_risk,a.origin,
        coalesce(feedback.signals,0)::int signals,
        coalesce(impact.open_problems,0)::int open_problems,
        coalesce(sources.source_names,'{}'::text[]) source_names
      FROM accounts a
      LEFT JOIN LATERAL (
        SELECT count(*)::int signals
        FROM feedback_items feedback
        WHERE feedback.org_id=a.org_id AND feedback.account_id=a.id
      ) feedback ON true
      LEFT JOIN LATERAL (
        SELECT count(DISTINCT account_impact.problem_id)::int open_problems
        FROM problem_account_impacts account_impact
        JOIN product_problems problem
          ON problem.org_id=account_impact.org_id
         AND problem.id=account_impact.problem_id
        WHERE account_impact.org_id=a.org_id
          AND account_impact.account_id=a.id
          AND problem.stage <> 'Closed'
      ) impact ON true
      LEFT JOIN LATERAL (
        SELECT array_agg(DISTINCT integration.provider ORDER BY integration.provider) source_names
        FROM account_source_links source_link
        JOIN integrations integration
          ON integration.org_id=source_link.org_id
         AND integration.id=source_link.integration_id
        WHERE source_link.org_id=a.org_id AND source_link.account_id=a.id
      ) sources ON true
      WHERE a.org_id=$1
        AND (
          a.origin <> 'demo'
          OR NOT EXISTS (
            SELECT 1
            FROM accounts live_account
            WHERE live_account.org_id=a.org_id
              AND live_account.origin <> 'demo'
          )
        )
  ...
  return { orgId,feedback,primaryProblem,recommendation,analytics,
    ...
    customers:customerResult.rows.map((item) => ({id:item.id,name:item.name,tier:item.tier,arr:item.arr,arrSource:item.arr_source,customerSince:item.customer_since,customerSinceKnown:item.customer_since_known,signals:item.signals,openProblems:item.open_problems,churnRisk:item.churn_risk,origin:item.origin,sourceNames:item.source_names})),
    settings };
}
