...
const correctiveSql = readFileSync(
  new URL("../../db/migrations/065_pdd_retry_constraint_name.sql", import.meta.url),
  "utf8",
);
...
describe("Prompt Testing retry migration", () => {
  ...
  it("drops PostgreSQL's truncated legacy constraint name", () => {
    expect(correctiveSql).toContain(
      "DROP CONSTRAINT IF EXISTS pdd_prompt_verifications_org_id_prompt_revision_id_story_ha_key",
    );
    expect(correctiveSql).not.toContain("DELETE FROM pdd_prompt_verifications");
  });
});
