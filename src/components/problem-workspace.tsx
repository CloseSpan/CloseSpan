...
export function ProblemWorkspace({ initialState, problem: primaryProblem, feedbackItems: feedback, investigation: recommendation }: { initialState: DemoState; problem: ProductProblem; feedbackItems: FeedbackItem[]; investigation: Recommendation }) {
  ...
  const [proposal, setProposal] = useState(recommendation.proposedAction);
  ...
  const revenue = evidence.reduce((sum, item) => sum + item.arr, 0);
  ...
  return <>
    ...
    <aside className="detail-stack">
      <section className="card"><div className="card-head"><h2>Impact score</h2><div className="score-ring" aria-label={`Impact score ${impact.score} out of 100`}><span>{impact.score}</span></div></div><div className="card-body">{primaryProblem.impactFactors.map((factor) => <div className="factor" key={factor.key}><span>{factor.label} · {factor.weight}%</span><div className="bar" role="meter" aria-label={`${factor.label} score`} aria-valuenow={factor.value} aria-valuemin={0} aria-valuemax={100}><span style={{ width: `${factor.value}%` }}/></div><strong>{factor.value}</strong></div>)}<div className="callout section-gap-sm"><div className="callout-title">Why this score</div><p className="subtle">{impact.explanation}</p></div><a className="btn full-width section-gap-xs" href="/settings#priority">Configure weights</a></div></section>

      <section className="card"><div className="card-head"><h2>Approval request</h2><span className={`badge ${state.approval.status === "Approved" ? "success" : state.approval.status === "Rejected" ? "high" : "medium"}`}>{state.approval.status}</span></div><div className="card-body"><h3>{state.approval.action}</h3><p className="subtle section-gap-xs">{state.approval.reason}</p><div className="callout section-gap-sm"><div className="callout-title"><ShieldCheck size={13}/> Action boundary</div><p className="subtle">Low risk · Reversible<br/>Shares redacted evidence with GitHub<br/><strong>Simulation only—no external request</strong></p></div>{editing && <label className="field section-gap-sm">Proposed action<textarea value={proposal} onChange={(event) => setProposal(event.target.value)} rows={5}/></label>}{state.approval.status === "Pending" ? <><button type="button" className="btn primary full-width section-gap-sm" disabled={busy} onClick={() => mutate("/api/workflow/approve")}>Approve & create simulated issue <ArrowRight size={14}/></button><div className="grid approval-secondary-actions"><button type="button" className="btn" onClick={() => setEditing((value) => !value)}>{editing ? "Save draft" : "Edit"}</button><button type="button" className="btn danger" disabled={busy} onClick={() => mutate("/api/workflow/reject")}>Reject</button></div></> : state.workItem ? <div className="callout section-gap-sm"><div className="callout-title">Work item created</div><p className="subtle"><FileCode2 size={13}/> {state.workItem.id} · Simulated GitHub issue</p></div> : null}{notice && <p className={`toast ${notice.kind}`} role={notice.kind === "error" ? "alert" : "status"}>{notice.message}</p>}</div></section>
      ...
      <section className="card"><div className="card-head"><h2>Engineering context</h2></div><div className="card-body"><div className="metric-label">Likely owner</div><strong>{primaryProblem.team}</strong><div className="metric-label section-gap-sm">Release correlation</div><strong>v4.18.2 · 87% confidence</strong><div className="metric-label section-gap-sm">Existing work</div><p className="subtle">No matching open issue found in seeded index.</p><a className="btn full-width section-gap-sm" href="/investigations">Open investigation <ExternalLink size={13}/></a></div></section>
    </aside></div>
  </>;
}
