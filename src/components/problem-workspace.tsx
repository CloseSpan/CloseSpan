...
export function ProblemWorkspace({ initialState, problem: primaryProblem, feedbackItems: feedback, investigation: recommendation }: { initialState: DemoState; problem: ProductProblem; feedbackItems: FeedbackItem[]; investigation: Recommendation }) {
  ...
  return <>
    ...
    <div className="two-col"><div className="detail-stack">
      ...
      <section className="card problem-audit-card"><div className="card-head"><h2>Activity & audit trail</h2><button type="button" className="btn" onClick={exportAudit}><Download size={13}/> Export CSV</button></div><div className="card-body problem-audit-scroll" role="region" aria-label="Activity and audit events" tabIndex={0}><ul className="timeline">{state.audit.map((item) => <li key={item.id}><strong>{item.actorName} · <time dateTime={item.occurredAt}>{new Date(item.occurredAt).toLocaleString()}</time></strong>{item.action}</li>)}</ul></div></section>
    </div>
