...
export default async function Page(){const user=await requireWorkspaceUser();const [state,data]=await Promise.all([findState(user.orgId),getWorkspaceData(user.orgId)]);const engineeringWorkflow=data.primaryProblem?await getEngineeringWorkflow(user.orgId,data.primaryProblem.id):null;return <><ApprovalsScreen initialState={state ? structuredClone(state) : null} problem={data.primaryProblem} investigation={data.recommendation} queue={data.investigationQueue}/>{engineeringWorkflow&&<EngineeringApprovalPanel orgId={user.orgId} initialWorkflow={structuredClone(engineeringWorkflow)}/>}</>}
